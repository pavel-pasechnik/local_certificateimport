...
$string['page:instructions'] = 'Выберите шаблон сертификата ниже, затем загрузите CSV в формате <code>userid,templateid,code,filename,timecreated</code> (оставьте колонки <code>templateid</code> и <code>code</code> пустыми — выбранный шаблон и tool_certificate заполнят их автоматически) и ZIP с PDF-файлами, где имена совпадают с колонкой <code>filename</code>. После импорта проверяйте выдачи через <code>tool/certificate/index.php</code>. Ниже можно скачать готовый шаблон CSV.';
...
$string['form:csvfile_help'] = 'CSV должен содержать столбцы: userid, templateid, code, filename, timecreated. Оставьте столбцы templateid и code пустыми, чтобы использовать выбор в форме и автонумерацию. Колонка timecreated может содержать UNIX-метку времени или обычную дату (YYYY-MM-DD, DD.MM.YYYY и т. п.).';
...
$string['error:csvcolumns'] = 'Строка {$a}: неверный формат (ожидается userid,templateid,code,filename,timecreated). Столбцы templateid и code могут быть пустыми, но обязаны присутствовать.';
