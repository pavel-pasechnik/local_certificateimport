...
$string['page:instructions'] = 'Виберіть шаблон сертифіката нижче, далі завантажте CSV у форматі <code>userid,templateid,code,filename,timecreated</code> (залиште стовпці <code>templateid</code> і <code>code</code> порожніми — вибраний шаблон і tool_certificate заповнять їх автоматично) та ZIP з PDF-файлами, імена яких збігаються зі стовпцем <code>filename</code>. Після імпорту перевіряйте видані сертифікати через <code>tool/certificate/index.php</code>. Нижче доступний готовий шаблон CSV.';
...
$string['form:csvfile_help'] = 'CSV повинен містити стовпці: userid, templateid, code, filename, timecreated. Залиште стовпці templateid і code порожніми, щоб скористатися вибором у формі та автонумерацією. Стовпець timecreated може містити UNIX-мітку часу або звичну дату (YYYY-MM-DD, DD.MM.YYYY тощо).';
...
$string['error:csvcolumns'] = 'Рядок {$a}: неправильний формат (очікується userid,templateid,code,filename,timecreated). Стовпці templateid і code можуть бути порожніми, але мають бути присутніми.';
